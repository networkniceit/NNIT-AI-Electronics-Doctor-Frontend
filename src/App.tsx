...
export default function App() {
  ...
  const blankTicket = { customerId:'', customerName:'', device:'', model:'', fault:'', status:'Open' as const, priority:'Medium' as const, cost:'', techNotes:'' };
  ...
  const blankInvoice = { ticketId:'', customerId:'', customerName:'', customerEmail:'', customerPhone:'', device:'', model:'', fault:'', laborCost:'', partsCost:'', totalCost:'', status:'Draft' as const, notes:'', dueDate:'' };
  ...
  const blankAppt = { customerId:'', customerName:'', ticketId:'', date:'', time:'', duration:'60', type:'Repair Drop-off', notes:'', status:'Scheduled' as const };
    ...
    try {
      ...
      if (res.data?.length > 0) {
        ...
        res.data.forEach((bt: any, idx: number) => {
          ...
          seenIds.add(newId);
          seenKeys.add(btKey);
          toAdd.push({ id: newId, customerId: bt.customer_id ?? '', customerName: bt.customer_name ?? '', device: bt.device ?? '', model: bt.model ?? '', fault: bt.fault ?? '', status: bt.status ?? 'Open', priority: bt.priority ?? 'Medium', cost: bt.cost ?? '', techNotes: bt.tech_notes ?? '', createdAt: bt.created_at ?? new Date().toLocaleString(), updatedAt: bt.updated_at ?? new Date().toLocaleString() });
        });
        if (toAdd.length > 0) {
